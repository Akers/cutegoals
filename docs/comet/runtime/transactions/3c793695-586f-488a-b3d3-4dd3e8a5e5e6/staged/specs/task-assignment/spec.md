...
所有时刻 MUST 以无歧义时刻保存，日历日期和未携带偏移量的本地时间 MUST 按实例时区解释，默认时区为 `Asia/Shanghai`。对未取消、当前状态不是 `APPROVED` 且任务类型（`snapshotTemplateTaskType`）不是 `REPEAT` 的分配，`overdue` MUST 在当前时刻严格晚于截止时间时派生为真；恰好等于截止时间时 MUST 为假。`REPEAT` 类型分配的生命周期由调度器经 `OPEN`→`EXPIRED` 状态流转管理，`overdue` MUST 恒为假，无论其截止时间是否已过；`REPEAT` 分配 MUST 不计入列表逾期筛选与月历逾期计数。逾期 MUST 仅为派生标记，不得自动删除分配、改写 `PENDING`、`SUBMITTED` 或 `REJECTED` 状态，也不得自动发放或扣除积分。每条分配 MUST 在创建时从家庭默认值固化有效迟交策略 `ALLOW` 或 `REJECT`，家长 SHALL 能够对单条未批准分配显式覆盖该策略；家庭默认值后续变化 MUST 仅影响新分配。

#### Scenario: 跨过截止时间边界
- **WHEN** 一条未取消、未批准且任务类型不是 `REPEAT` 的分配，其当前时刻从等于截止时间推进到晚于截止时间
- **THEN** `overdue` 从假变为真，而分配审核状态和历史保持不变

#### Scenario: 重复任务过期后不标记逾期
- **WHEN** 一条 `REPEAT` 分配的截止时间已过且当前状态不是 `APPROVED`（包括已被调度器置为 `EXPIRED` 的分配）
- **THEN** `overdue` 恒为假，该分配不出现在逾期筛选结果中，也不计入月历当日逾期计数；分配状态与历史保持不变
...
列表和详情响应 MUST 对 `REPEAT` 类型分配附带「同一孩子、同一模板」维度的进度聚合字段：`approvedSubmissionCount`（该范围内审核记录中 `decision='APPROVED'` 的去重计数）与 `earnedPoints`（该范围内积分流水 `type='EARN'` 的 `amount` 总和，`REFUND` 不扣减）；两字段的统计口径 MUST 与重复提交前置校验一致。聚合查询 MUST 以批量方式执行，不得随页内分配数量逐条发起查询。非 `REPEAT` 类型分配的这两个字段 MAY 为空。
...
- **WHEN** 家长查询包含 `REPEAT` 分配的列表，该孩子在该模板上已有 2 次 `APPROVED` 审核、累计 `EARN` 积分 30
- **THEN** 该分配响应包含 `approvedSubmissionCount=2`、`earnedPoints=30`，与既有 `snapshotTemplateMaxSubmissions`、`snapshotTemplatePointsCap` 快照字段并列
