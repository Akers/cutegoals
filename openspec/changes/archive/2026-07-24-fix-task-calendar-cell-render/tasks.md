...
## 6. verify-fail #2：Badge 绝对定位（消除 Badge 撑高行）

- [x] 6.1 根因确认（explorer ses_072514f19ffemto 在真实 Chromium 实测）：antd `<Badge size="small">` wrapper 是 `display: inline-block` + `vertical-align: middle`，含 Badge 的 td 被撑到 **61px**（无 Badge 的 td 仅 **36px**，撑高 +25px）。`.ant-picker-content table height=259px` 未强制 6 行均分，而是按 min-content 自然分布。
- [x] 6.2 修复 `TaskCalendar.tsx`：移除 antd `<Badge>` 组件，替换为自定义 `<span>` 角标（`position: absolute; top: 0; left: 0; fontSize: 10; borderRadius: 8`），脱离正常 flow 不撑高 cell。
- [x] 6.3 更新测试：`.ant-badge` → `[data-testid^="task-badge-"]`；新增断言验证 position: absolute + fontSize: 10px。
- [x] 6.4 跑 `pnpm --filter web test` 转绿：18 files / **181 tests** 全部通过。
- [x] 6.5 跑 `pnpm --filter web build`：成功 6.99s exit 0。
- [x] 6.6 prettier：两个文件 unchanged，重跑测试 181/181 仍绿。
- [x] 6.7 待用户浏览器手工验证（在 verify → archive 归档前最终确认环节完成）：`pnpm --filter web dev` → `/parent/tasks`，查看有任务数的月份（含 Badge 行）是否不再错位。含 Badge 与不含 Badge 的 td 行高应一致（≈36px）。
