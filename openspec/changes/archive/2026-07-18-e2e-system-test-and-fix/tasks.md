...
- [x] 1.1 启动 Redis 容器（`deploy/docker-compose.yml` 的 `mit-modelide-core-redis` 服务），确认 :6379 监听
- [x] 1.2 确认 PostgreSQL :35432 可连接（用户 cutegoals / 库 cutegoals / schema cutegoals）
- [x] 1.3 运行 `scripts/reset-dev-db.sh` 重置数据库到干净初始状态，记录重置前后表清单作为证据
- [x] 1.4 启动后端：`cd server && mvn clean -pl web -am spring-boot:run -DskipTests`，确认 :8080 监听且 `/api/health` 返回 UP
- [x] 1.5 启动前端：`cd web && npm install && npm run dev`，确认 :5173 监听；记录实际是 Vite 还是 UmiJS（从 `web/package.json` 与启动日志判定）
- [x] 1.6 验证前端 `/api` 代理到 :8080 正常工作（任意未登录请求转发即可）

## 2. 测试计划制定

- [x] 2.1 在 `openspec/changes/e2e-system-test-and-fix/reports/test-plan.md` 写入完整测试计划，覆盖 admin / parent / child 三端所有 UI 可达功能
- [x] 2.2 计划中显式列入核心不变量验证用例：积分/库存非负、积分流水/审核历史/兑换快照不可变、审核/兑换/退款幂等与事务原子
- [x] 2.3 计划中显式列入安全基线用例：明文密码/PIN/令牌/完整手机号不出现在 API 响应、浏览器存储、后端日志
- [x] 2.4 计划按模块（auth/family/task/task-review/points/prize/exchange/instance-management/admin）分组，每模块至少含一条正常路径 + 一条边界用例
- [x] 2.5 计划中标注每个用例的预期结果与判定证据（截图 / API 响应 / DB 状态）

## 3. 第一轮：agent-browser 全量测试

- [x] 3.1 准备 agent-browser：加载 skill、确认 CLI 可用、规划三套独立 browser context（admin / parent / child）
- [x] 3.2 admin 端测试：使用 13600049114/117315Akers 登录 `/admin`，覆盖概览、账号管理、审计日志、健康检查四个功能
- [x] 3.3 parent 端测试 - 家庭：登录 `/parent`，覆盖家庭信息查看/编辑、添加孩子档案（cici）、添加家长账号、设备授权流程
- [x] 3.4 parent 端测试 - 任务：发布任务模板、查看任务列表、查看孩子任务完成情况
- [x] 3.5 parent 端测试 - 审核：对孩子提交的任务执行通过/驳回、验证幂等（重复审核不产生多次积分）
- [x] 3.6 parent 端测试 - 积分：查看积分流水、调整积分、验证余额非负与流水不可变
- [x] 3.7 parent 端测试 - 奖品：创建奖品、创建盲盒、库存管理
- [x] 3.8 parent 端测试 - 兑换：查看兑换申请、核销、验证核销幂等与库存扣减原子
- [x] 3.9 child 端测试：用 cici/PIN 180614 登录 `/child`，覆盖今日任务查看/提交、奖品兑换、盲盒抽取、历史记录
- [x] 3.10 跨模块端到端：孩子提交任务 → 家长审核 → 积分入账 → 孩子兑换奖品 → 家长核销，验证全链路状态一致
- [x] 3.11 不变量验证用例：尝试构造余额为负、重复审核、并发兑换等场景，验证系统拒绝或正确处理
- [x] 3.12 安全基线用例：登录、敏感操作期间观察浏览器存储与后端日志，验证不出现明文敏感字段
- [x] 3.13 整理第一轮发现的全部缺陷到 `reports/round-1.md`，按 Critical/High/Medium/Low/Cosmetic 分级，每条含编号、模块、复现步骤、预期/实际、证据
- [x] 3.14 为每个 Critical/High 缺陷单独创建 `reports/bug-NNN-<slug>.md`，记录根因初判
...
- [x] 6.3 运行 `comet guard e2e-system-test-and-fix verify --apply` 进入 verify 阶段
- [x] 6.4 按 comet-verify 流程完成验证报告，准备归档
