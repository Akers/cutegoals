...
- [x] 1.4 简化 Grid 容器：`.task-calendar-grid` 样式从 `display: grid; grid-template-columns: repeat(auto-fit, minmax(400px, 1fr))` 改为 `display: block; width: 100%`；移除 `<style>` 块中的 `@media (max-width: 767px)` 响应式覆盖
