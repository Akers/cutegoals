#!/usr/bin/env node
/**
 * 检查 src/design 下的 CSS 文件是否被 src 下其他源码（.ts / .tsx / .css）通过
 * import 或 @import 引用。未引用则 exit 1，预防 “CSS 写了但没 import” 回归
 * （如 fix-global-styles-not-loaded 暴露的根因）。
 *
 * 检测规则：
 *   - JS/TS: `import '@/design/index.css'` / `import x from './x.css'` / `from './x.css'`
...
const STYLES_DIR = fileURLToPath(new URL('design/', new URL('../src/', import.meta.url)));
...
if (cssFiles.length === 0) {
  console.error('✗ No CSS files found in src/design/');
  process.exit(1);
}
...
if (orphans.length > 0) {
  console.error(
    `✗ ${orphans.length} CSS file(s) in src/design/ not imported by any source:`,
  );
  ...
  console.error('');
  console.error(
    "Fix: import these files via `import '@/design/<file>'` in a TS/TSX module,",
  );
